...
export function PwaUpdatePrompt() {
    ...
    const showUpdateNoticeToast = useCallback(() => {
        if (updateNoticeShownRef.current) return;

        updateNoticeShownRef.current = true;
        addToast({
            message: 'Hay una nueva versión disponible. Preparando actualización',
            variant: 'info',
            durationMs: 8000,
        });
    }, [addToast]);
