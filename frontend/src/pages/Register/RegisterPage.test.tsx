...
describe('RegisterPage', () => {
  ...
  it('shows Spanish consent and password feedback before calling the API', async () => {
    ...
    expect(screen.getByLabelText('Requisitos de contraseña')).toBeVisible();
  ...
  it('maps server field validation details to visible form errors', async () => {
    ...
    await user.click(screen.getByLabelText('Confirmo que tengo 18 años o más'));
    await user.click(screen.getByRole('button', { name: 'Crear cuenta' }));
  ...
  it('shows an email confirmation screen after a valid registration', async () => {
    const user = userEvent.setup();
    ...
    expect(await screen.findByRole('heading', { name: 'Revisa tu correo' })).toBeVisible();
