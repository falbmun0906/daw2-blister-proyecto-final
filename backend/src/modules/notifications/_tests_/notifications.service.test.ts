...
describe('notifications.service', () => {
  ...
  it('hydrates legacy follow-up reminders with the appointment title and linked treatment', async () => {
    const owner = await createUser('notify-legacy-appointment-owner');
    const blister = await BlisterModel.create({
      name: 'Familia',
      members: [{ userId: owner._id, role: 'OWNER' }],
    });
    ...
    await NotificationModel.create({
      ...
      severity: 'info',
      title: '¿Qué tal ha ido la cita?',
      message: 'Tras la cita, revisa si hay cambios que aplicar al tratamiento.',
      metadata: {
    ...
    expect(result.notifications).toHaveLength(1);
    expect(result.notifications[0]?.message).toBe("Tras la cita 'Digestivo', revisa si hay cambios que aplicar al tratamiento.");
    expect(result.notifications[0]?.metadata?.appointmentTitle).toBe('Digestivo');
    expect(result.notifications[0]?.metadata?.treatmentId).toBe(treatment._id.toString());
  });
